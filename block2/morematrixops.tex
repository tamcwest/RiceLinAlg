...
\edXsolution{ 
 Multiply on both sides of the equation by $AD\inv$ from the left, and then by $CD$ from the right.
...
\edXsolution{
Our matrix $E$ scales a single row of $A$, so it's similar to the identity matrix, but with the necessary scalar 4 in its own last row.
}
...
\edXsolution{
This is a permutation matrix; it swaps the first and second rows of $A$, so it is simply the identity matrix, but with its first and second rows swapped.
}
...
Suppose $A$ is a $4\times 3$ matrix, and $B$ is a $3 \times 5$ matrix, so that $AB$ is a $4\times 5$ matrix.  
...
\edXsolution{ 
One possible procedure: left-multiply on both sides of the equation by $(B^t)\inv$. Take the transpose of both sides. Then you have $A\inv = (C\inv)^t D B\inv$, and you can take the inverse of both sides.
...
\edXsolution{
Because one of the rows is a linear combination of the others, the rows are not linearly independent. If we row reduce this matrix, we will get a row of zeros, not the identity matrix. Therefore $T$ is not onto, $A$ has a free variable, etc. Note that the matrix equation stated above does not necessarily have more than one solution, because the vector on the right hand side might not be in the span of the columns of $A$ (i.e. the equation could have no solution). 
}
