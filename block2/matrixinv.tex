...
\edXsolution{ 	If $n<m$, our codomain is greater than our domain. Therefore, it may be true that $T$ maps any distinct $u$, $v$ $\in V$ to different points ($T(u) \neq T(v)$); i.e., our codomain will accommodate this. So $T$ might be into. However, this is merely allowed, not required, by the circumstances. ($T$ could map everything to zero, for example.) So $T$ might or might not be into.
	
	$T$ can only map each point of the domain to a single point in the codomain. We see that with $n<m$, the image of the domain can't equal the codomain. $T$ can't be onto.
	
	If $n>m$, our domain is greater than our codomain. Every point in our domain must be mapped to something in the codomain by $T$. Therefore we must have at least one pair of distinct points $u$, $v$ $\in V$ such that $T(u) = T(v)$. $T$ can't be into.
	
	Furthermore, if $n>m$, we have sufficient points in our domain to map to every point in the codomain, so the image of the domain could be the codomain. However, this is merely allowed by the circumstances, not required, so $T$ might or might not be onto.
}
...
\edXsolution{ We've seen that, if $n<m$ or if $n>m$, T can't be both into and onto. Therefore, in neither case could T be invertible. }
...
\edXsolution{
Row reduce $\left[ \begin{array}{cc} A  \\  I \end{array} \right] \]$ to get the inverse.
}
